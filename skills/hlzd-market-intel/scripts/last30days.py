...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--extract-pain", action="store_true",
                        help="[HLZD fork] Extract Top 5 customer pain points from evidence. "
                             "Output: --pain-output PATH (optional) writes raw JSON; otherwise "
                             "pain data is embedded in the quality nudge text.")
    parser.add_argument("--pain-output", help="[HLZD fork] Path to write pain extraction JSON")
    parser.add_argument("--pain-profile",
                        help="[HLZD fork] Path to a YAML/JSON topic-cluster profile "
                             "(overrides default security-camera profile). "
                             "See presets/pain-profiles/ for examples (e.g. biomedical.yaml).")
...
def main() -> int:
    ...
    if not args.hiring_signals:
        try:
            ...
            # HLZD fork: extract + inject pain data (before compute_quality_score
            # so quality_nudge sees research_results["pain_extraction"]).
            # run_pain_pipeline centralizes extract + pain_output JSON + inject,
            # and is idempotent (caches into research_results).
            pain_data = None
            if getattr(args, "extract_pain", False):
                from lib import pain_extractor
                pain_data = pain_extractor.run_pain_pipeline(
                    args, research_results, report.items_by_source
                )
            ...
            # HLZD fork: render pain section to stderr (reuses cached pain_data,
            # does NOT re-extract). Kept separate from run_pain_pipeline so
            # quality scoring runs between inject and render.
            if pain_data and pain_data.get("top_5_pains"):
                from lib import pain_extractor
                sys.stderr.write(f"\n{pain_extractor.format_pain_section(pain_data)}\n")
                sys.stderr.flush()
        except Exception:
            pass
